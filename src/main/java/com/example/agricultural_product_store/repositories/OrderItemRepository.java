...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
    @Query(value = "SELECT datefield as dayStr, IFNULL(SUM(quantity), 0) as total FROM calendar LEFT JOIN order_item ON date_format(datefield, '%Y-%m-%d') = date_format(create_time, '%Y-%m-%d') AND product_id = :productId WHERE datefield BETWEEN '2023-06-01' AND curdate() GROUP BY datefield", nativeQuery = true)
